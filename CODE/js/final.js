d3.json("out.json")
  .then((data) => {
    ...
    function createRadarChart(clusterData, clusterIndex) {
      ...
      clusterData.representative_shapes.forEach((shape, shapeIndex) => {
        const shapeColor = d3.schemeSet3[shapeIndex % 12];
        const radarLine = d3.lineRadial();

        const pathData = radarLine(
          shape.coordinates.map((coord, i) => [
            angleSlice * i,
            rScale(Math.abs(coord)),
          ])
        );
        ...
        svg
          .selectAll(`.radarLine-${shapeIndex}`)
          .data(shape.coordinates)
          .enter()
          .append("circle")
          .attr("class", `radarLine-${shapeIndex}`)
          .attr("r", 3)
          .attr(
            "cx",
            (coord, i) =>
              rScale(Math.abs(coord)) * Math.cos(angleSlice * i - Math.PI / 2)
          )
          .attr(
            "cy",
            (coord, i) =>
              rScale(Math.abs(coord)) * Math.sin(angleSlice * i - Math.PI / 2)
          )
          .style("fill", shapeColor)
